...
export function buildMapEntry(buildContext: BuildContext): MapEntry {
  const inputValue = fetchRequiredNode<NumberLiteral>(buildContext, NodeKind.NUMBER_LITERAL);
  const commaPunctuator = fetchRequiredNode<Token>(buildContext, NodeKind.TOKEN, TokenKind.COMMA);
  const outputValue = fetchRequiredNode<AggregateOutputValue>(
    buildContext,
    NodeKind.AGGREGATE_OUTPUT_VALUE,
  );

  return new MapEntry(inputValue, commaPunctuator, outputValue, [
    inputValue,
    commaPunctuator,
    outputValue,
  ]);
