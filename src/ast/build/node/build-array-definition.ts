...
export function buildArrayDefinition(buildContext: BuildContext): ArrayDefinition {
  ...
  if (elementaryType) {
    ...
    lengthAttribute = fetchOptionalNode<LengthAttribute>(buildContext, NodeKind.LENGTH_ATTRIBUTE);
    if (lengthAttribute) {
      children.push(lengthAttribute);
    } else {
      // If an elementary type is present, a length attribute must be present
      throw new InternalScannerError("Expected LengthAttribute node after ElementaryType node");
    }
  } else {
  ...
  let dimensions: ZeroToManyList<ExplicitArrayDimension | PartialArrayDimension> = [];
  ...
  if (implicitArrayDimension) {
    ...
    dimensions = fetchZeroToManyList<ExplicitArrayDimension | PartialArrayDimension>(
      buildContext,
      NodeKind.ARRAY_DIMENSION,
