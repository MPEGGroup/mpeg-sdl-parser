...
export function buildSwitchStatement(buildContext: BuildContext): SwitchStatement {
  const children: Array<AbstractNode> = [];

  const switchKeyword = fetchRequiredNode<Token>(buildContext, NodeKind.TOKEN, TokenKind.SWITCH);
  ...
  const expression = fetchRequiredNode<AbstractExpression | NumberLiteral | Identifier>(
    buildContext,
  ...
  const caseClauses = fetchZeroToManyList<CaseClause>(buildContext, NodeKind.CASE_CLAUSE);
  children.push(...caseClauses);
  const defaultClause = fetchOptionalNode<DefaultClause>(buildContext, NodeKind.DEFAULT_CLAUSE);
  if (defaultClause) {
