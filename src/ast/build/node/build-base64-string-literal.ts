...
export function buildBase64StringLiteral(buildContext: BuildContext): StringLiteral {
  ...
  for (const literal of literals) {
    if (isToken(literal) && literal.tokenKind === TokenKind.BASE64_STRING_LITERAL_CHARACTERS) {
      value += literal.text;
    }
  }
  value = value
  ...
  return new StringLiteral(StringLiteralKind.BASIC, value, literals, [...literals]);
}
